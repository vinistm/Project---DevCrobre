...
describe("Testing GET method in /agreement/:id", () => {
  ...
  describe("Creating Debts", () => {
    test("f) POST /debts : Create Debts01", async () => {
      const response = await request(app)
        .post("/debts")
        .send(testDebts)
        .set("Authorization", `Bearer ${testAdm.token}`);
  ...
  describe("Listing One Agreement", () => {
    test("h) GET /agreement/:id : List One Agreement", async () => {
      const response = await request(app)
        .get("/agreement/" + testAgreement.id)
        .set("Authorization", `Bearer ${testAdm.token}`);
      expect(response.status).toEqual(200);
      expect(response.body).toHaveProperty("id");
    });
