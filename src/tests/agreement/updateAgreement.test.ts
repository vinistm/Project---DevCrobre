...
describe("Testing PATCH method in /agreement/:id", () => {
  ...
  describe("Updating Agreement", () => {
    test("g) PATCH /agreement : Update Agreement", async () => {
      const response = await request(app)
        .patch("/agreement/" + testAgreement.id)
        .send({ ...testAgreement, agreedValue: 1000 })
        .set("Authorization", `Bearer ${testAdm.token}`);
